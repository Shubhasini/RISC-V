//url: https://myth1.makerchip.com/sandbox/02kfkhByV/00ghGYB
//Day-5 slide-33 3-cycle valid
...
\TLV
   ...
   |cpu
      @0
         $reset = *reset;
         //-------3 cycle valid------
         $start = >>1$reset && !$reset; //after reset & at inactive reset
         $valid = $reset ? 1'b0 
                         : $start ? 1'b1
                         : >>3$valid;
         //------------------------------
         $pc[31:0] = >>1$reset ? 32'b0 
                               : >>1$taken_br ? >>1$br_tgt_pc   //modified pc for branch
                               : >>1$pc + 32'd4;  //next pc value
      @1
         $imem_rd_en = !$reset;
         ...
         $src1_value[31:0] = $rf_rd_data1; //reading data
